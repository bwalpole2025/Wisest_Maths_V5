      ...
      <div className="glass sticky top-24 z-30 flex flex-col gap-3 rounded-2xl p-3 sm:flex-row sm:items-center">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full rounded-xl border border-white/10 bg-white/[0.03] py-2.5 pl-10 pr-4 text-sm text-foreground transition-colors placeholder:text-muted-foreground focus:border-violet-400/50 focus:bg-white/[0.05]"
          />
        </div>
        <div
          role="group"
          aria-label="Filter by difficulty"
          className="flex items-center gap-1 rounded-xl border border-white/[0.06] bg-white/[0.02] p-1"
        >
          <SlidersHorizontal className="mx-1.5 h-3.5 w-3.5 text-muted-foreground" aria-hidden />
          {FILTERS.map((f) => (
            <button
              key={f}
              type="button"
            ...
            >
              {difficulty === f && (
                <motion.span
                  ...
                  className="absolute inset-0 rounded-lg bg-white/[0.09] ring-1 ring-white/10"
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3 p-4 sm:px-5">
                ...
                <DifficultyBadge level={q.difficulty} className="shrink-0" />
